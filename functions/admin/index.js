...
.topbar { background: var(--surface); border-bottom: 1px solid var(--border); padding: 12px 16px; display: flex; align-items: center; justify-content: space-between; gap: 12px; position: sticky; top: 0; z-index: 10; }
.topbar-title { font-size: 18px; font-weight: 700; letter-spacing: 0.05em; }
.topbar-actions { display: flex; gap: 8px; }
.main { flex: 1; padding: 16px; max-width: 800px; margin: 0 auto; width: 100%; }

/* ── Buttons ── */
button, .btn { display: inline-flex; align-items: center; gap: 6px; padding: 9px 16px; border: none; border-radius: var(--radius); font-family: var(--font); font-size: 14px; font-weight: 600; cursor: pointer; transition: opacity .15s; text-decoration: none; white-space: nowrap; }
...
input[type=text], input[type=date], input[type=password], textarea, select {
  ...
  border-radius: var(--radius); color: var(--text); font-family: var(--font); font-size: 15px;
}
...
.login-error { color: var(--danger); font-size: 14px; margin-top: 10px; display: none; }

/* ── Post list ── */
.post-list { display: flex; flex-direction: column; gap: 10px; }
.post-item { background: var(--surface); border-radius: var(--radius); padding: 14px 16px; display: flex; align-items: center; justify-content: space-between; gap: 12px; cursor: pointer; border: 1px solid var(--border); transition: border-color .15s; }
.post-item:hover { border-color: var(--accent); }
.post-item-info { flex: 1; min-width: 0; }
.post-item-title { font-size: 16px; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.post-item-meta { font-size: 13px; color: var(--muted); margin-top: 3px; }
...
.badge-draft { background: var(--draft); color: #fff; }
.badge-published { background: var(--published); color: #fff; }
.badge-edited { background: var(--edited); color: #000; }
.empty-state { text-align: center; padding: 60px 20px; color: var(--muted); }
.empty-state p { margin-bottom: 20px; }

/* ── Editor ── */
.editor-header { display: flex; align-items: center; gap: 10px; margin-bottom: 20px; flex-wrap: wrap; }
.editor-header h2 { flex: 1; font-size: 20px; }
.editor-actions { display: flex; gap: 8px; flex-wrap: wrap; }
textarea#content { min-height: 320px; font-family: 'Courier New', monospace; font-size: 14px; line-height: 1.6; resize: vertical; }
.toolbar { display: flex; gap: 6px; margin-bottom: 8px; flex-wrap: wrap; }
.toolbar button { padding: 5px 10px; font-size: 13px; background: var(--surface2); color: var(--text); border-radius: 4px; }
.tabs { display: flex; border-bottom: 1px solid var(--border); margin-bottom: 16px; gap: 0; }
.tab { padding: 8px 16px; font-size: 14px; font-weight: 600; cursor: pointer; color: var(--muted); border-bottom: 2px solid transparent; margin-bottom: -1px; background: none; border-radius: 0; }
...
.preview code { background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 3px; font-family: monospace; }
.preview pre { background: rgba(0,0,0,0.3); padding: 16px; border-radius: var(--radius); overflow-x: auto; margin: 1em 0; }
.preview blockquote { border-left: 3px solid var(--accent); padding-left: 16px; color: var(--muted); margin: 1em 0; }
.preview ul, .preview ol { padding-left: 24px; margin-bottom: 1em; }
.preview li { margin-bottom: 0.3em; line-height: 1.7; }
.danger-zone { margin-top: 32px; padding-top: 20px; border-top: 1px solid var(--border); display: flex; gap: 8px; flex-wrap: wrap; align-items: center; }
.danger-zone span { font-size: 13px; color: var(--muted); flex: 1; }
.status-bar { padding: 8px 0; font-size: 13px; color: var(--muted); min-height: 28px; }
...
<div id="app">
  ...
  <div id="main-screen" style="display:none">
    <div class="topbar">
      <span class="topbar-title">Blog Admin</span>
...
async function showList() {
  ...
  view.innerHTML = \`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
      <h2 style="font-size:22px">Posts</h2>
      <button class="btn btn-primary btn-sm" onclick="showNew()">+ New post</button>
    </div>
    \${posts.length === 0
      ? \`<div class="empty-state"><p>No posts yet.</p><button class="btn btn-primary" onclick="showNew()">Write your first post</button></div>\`
...
function postItem(p) {
  ...
  return \`<div class="post-item" onclick="showEditor('\${p.slug}')">
    <div class="post-item-info">
      ...
      <div class="post-item-meta">\${p.date} \${p.tags?.length ? '· ' + p.tags.map(t => '#'+t).join(' ') : ''}</div>
    </div>
    \${badge}
  </div>\`;
}
...
async function showEditor(slug) {
  ...
  view.innerHTML = \`
    ...
    <div class="row">
      <div class="field"><label>Slug</label><input type="text" id="ed-slug" value="\${esc(post.slug)}" readonly style="opacity:.6;cursor:not-allowed"></div>
    ...
    <div id="panel-write">
      ...
      <textarea id="content" spellcheck="true" placeholder="Write in markdown…">\${esc(post.markdown || '')}</textarea>
...
async function saveDraft() {
  ...
  if (r.ok) { setStatus('ed-status', 'Saved ✓'); } else { setStatus('ed-status', 'Save failed', true); }
}
...
async function unpublish() {
  if (!confirm('Unpublish this post? It will be removed from the public blog.')) return;
